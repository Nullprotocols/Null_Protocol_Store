# main.py - FINAL PRODUCTION BOT - POSTGRESQL + CUSTOM KEYS + BACKUP + ALL FIXES
...
async def log_key_gen(user_id, key, api_type):
    ...
    text = f"🔑 New Key\n👤 {user.get('first_name','')} (@{user.get('username','')}) [{user_id}]\n🔹 {api_type.upper()}\n<code>{key}</code>"
...
async def start(update, context):
    ...
    if 'pending_ref' in context.user_data:
        r = context.user_data.pop('pending_ref')
        await add_credits(r, REFERRAL_REWARD_CREDITS)
        try: await application.bot.send_message(r, f"🎉 +{REFERRAL_REWARD_CREDITS} credits referral")
        except: pass
...
async def check_join_cb(update, context):
    ...
    if joined:
        async with pool.acquire() as conn:
            await conn.execute("UPDATE users SET joined_force_channels=TRUE WHERE user_id=$1", uid)
        await q.edit_message_text("✅ Thank you! Press /start")
    else:
...
async def gen_specific_key(update, context):
    ...
    await q.edit_message_text(f"✅ Key: <code>{key}</code>\nExample:\n<code>{ep}</code>", parse_mode='HTML', reply_markup=InlineKeyboardMarkup([[back_btn("menu_genkey")]]))
...
async def buy_plan(update, context):
    ...
    if await is_admin(uid):
        ...
        if pl:
            async with pool.acquire() as conn:
                start = datetime.now(); end = start + timedelta(days=pl['duration_days'])
...
async def referral_menu(update, context):
    ...
    await q.edit_message_text(f"🔗 Link:\n<code>{link}</code>\nEarn {REFERRAL_REWARD_CREDITS} credits.", parse_mode='HTML', reply_markup=InlineKeyboardMarkup([[back_btn("menu_start")]]))
...
async def show_admin_list(update, context, page):
    ...
    for a in admins:
        ...
        if uid != OWNER_ID:  # can't demote the main owner
...
async def toggle_ban(update, context):
    ...
    # Refresh user list (go back to page 0 for simplicity)
...
async def toggle_admin(update, context):
    q = update.callback_query; await q.answer()
    uid = int(q.data.split('_')[1])
    is_owner = await is_admin(uid) and uid != OWNER_ID
    if uid == OWNER_ID:
...
async def handle_text(update, context):
    ...
    if state == 'awaiting_user_for_credits':
        ...
        if not key.startswith('ak_') or len(key) < 10:
            await update.message.reply_text("Invalid key format. Must start with 'ak_' and length >= 10."); return
        exists = await pool.fetchval("SELECT key FROM api_keys WHERE key=$1", key)
        if exists:
            await update.message.reply_text("Key already exists."); return
        ...
        try: days = int(text); context.user_data['cust_expiry'] = days
...
async def handle_broadcast_media(update, context):
    ...
    await update.message.reply_text(f"✅ Broadcast to {success}/{len(users)}.")
...
async def on_startup():
    ...
    if BOT_MODE == "webhook":
        ...
        logger.info("Polling started (PTB 21.x)")
